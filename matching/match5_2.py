...
class PDFExcelTool:
    ...
    def extract_pdf_title_block(self, pdf_path):
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                # 打印提取的文本内容
                print(f"\n===== PDF文件 {os.path.basename(pdf_path)} 提取的文本内容 =====")
                print(extracted_text if extracted_text else "未提取到任何文本内容")
                print("===========================================\n")
                ...
                # 表格搜索函数（保持不变）
                def find_in_grid(keywords, search_range=4, ignore_values=[]):
                    matches = []
                ...
                if tables:
                    ...
                    # 打印合并后的表格内容
                    print(f"===== PDF文件 {os.path.basename(pdf_path)} 合并后的表格内容 =====")
                    if table_grid:
    ...
    def normalize_description(self, desc):
        """规范化描述字符串以便比较（保留颜色处理逻辑，严格校验其他格式）"""
        # 1. 标准化分隔符
        desc = re.sub(r'[，；、]', ',', desc)
